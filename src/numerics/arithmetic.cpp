...
namespace dft_core::numerics::arithmetic::summation {
  ...
  const summation::Type& CompensatedSum::type() const {
    return this->type_;
  }

  CompensatedSum::CompensatedSum(summation::Type type) : type_(type) {
  ...
  x_type CompensatedSum::sum() const {
    if (this->type_ == summation::Type::KahanBabuska) {
      return this->sum_;
    }
  ...
  CompensatedSum& CompensatedSum::operator=(x_type x) {
    sum_ = x;
    error_ = (type_ == summation::Type::KahanBabuskaKlein) ? error_type{0.0, 0.0} : error_type{0.0};
    return *this;
  }
