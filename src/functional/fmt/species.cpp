...
namespace dft::functional::fmt {
  ...
  Measures Species::measures_at(arma::uword index) const {
    Measures m;
    double r = 0.5 * diameter_;
    double inv_4pi_r = 1.0 / (4.0 * std::numbers::pi * r);
    double inv_4pi_r2 = inv_4pi_r / r;

    m.eta = weights_.eta.field()(index);
    m.n2 = weights_.scalar.field()(index);
    m.n1 = m.n2 * inv_4pi_r;
    m.n0 = m.n2 * inv_4pi_r2;

    for (int a = 0; a < 3; ++a) {
      m.v1(a) = weights_.vector[a].field()(index);
    }
    m.v0 = m.v1 * inv_4pi_r;

    for (int i = 0; i < 3; ++i)
      for (int j = i; j < 3; ++j) {
        m.T(i, j) = weights_.tensor(i, j).field()(index);
        m.T(j, i) = m.T(i, j);
      }
  ...
  void Species::set_derivatives(const Measures& dm, arma::uword index, bool tensor) {
    double r = 0.5 * diameter_;
    double inv_4pi_r = 1.0 / (4.0 * std::numbers::pi * r);
    double inv_4pi_r2 = inv_4pi_r / r;

    weights_.eta.derivative()(index) = dm.eta;

    // Chain rule: d/d(scalar) = dPhi/dn0 / (4piR^2) + dPhi/dn1 / (4piR) + dPhi/dn2
    weights_.scalar.derivative()(index) = dm.n0 * inv_4pi_r2 + dm.n1 * inv_4pi_r + dm.n2;

    // Chain rule: d/d(vector_a) = dPhi/dv0_a / (4piR) + dPhi/dv1_a
    arma::rowvec3 dv = dm.v0 * inv_4pi_r + dm.v1;
    for (int a = 0; a < 3; ++a) {
      weights_.vector[a].derivative()(index) = dv(a);
    }
    ...
    if (tensor) {
      for (int i = 0; i < 3; ++i) {
        for (int j = i; j < 3; ++j) {
          ...
          weights_.tensor(i, j).derivative()(index) = val;
        }
      }
    }
  }
  ...
  void Species::accumulate_forces(bool tensor) {
    ...
    math::fourier::FourierTransform force_fft(shape);
    force_fft.zeros();
  ...
  double Species::compute_free_energy(const FMT& functional) {
  ...
  double Species::compute_forces(const FMT& functional) {
    bool tensor = functional.needs_tensor();
    convolve_density(tensor);
