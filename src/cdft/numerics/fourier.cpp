...
namespace cdft::numerics {
  ...
  FourierTransform::FourierTransform(std::vector<long> shape) : shape_(std::move(shape)) {
    ...
    forward_ = fftw_plan_dft_r2c_3d(
        static_cast<int>(shape_[0]), static_cast<int>(shape_[1]), static_cast<int>(shape_[2]),
        real_data_, fourier_data_, FFTW_ESTIMATE);

    backward_ = fftw_plan_dft_c2r_3d(
        static_cast<int>(shape_[0]), static_cast<int>(shape_[1]), static_cast<int>(shape_[2]),
        fourier_data_, real_data_, FFTW_ESTIMATE);

    zeros();
  }

  FourierTransform::~FourierTransform() { release(); }
  ...
  const std::vector<long>& FourierTransform::shape() const { return shape_; }
  ...
  void FourierTransform::forward() { fftw_execute(forward_); }
  void FourierTransform::backward() { fftw_execute(backward_); }
  ...
  std::span<double> FourierConvolution::input_a() { return a_.real(); }
  std::span<double> FourierConvolution::input_b() { return b_.real(); }
  std::span<const double> FourierConvolution::result() const { return c_.real(); }
  ...
  const std::vector<long>& FourierConvolution::shape() const { return a_.shape(); }
  long FourierConvolution::total() const { return a_.total(); }
