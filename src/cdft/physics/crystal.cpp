...
namespace cdft::physics {
  ...
  namespace {
    ...
    void scale_positions(std::vector<Pos3>& atoms, const Pos3& L) {
      for (auto& a : atoms) { a[0] *= L[0]; a[1] *= L[1]; a[2] *= L[2]; }
    }

    void rotate_y_to_z(std::vector<Pos3>& atoms) {
      for (auto& a : atoms) { double t = a[2]; a[2] = a[1]; a[1] = -t; }
    }

    void rotate_x_to_z(std::vector<Pos3>& atoms) {
      for (auto& a : atoms) { double t = a[2]; a[2] = a[0]; a[0] = -t; }
    }
    ...
    struct UnitCell { std::vector<Pos3> atoms; Pos3 dims{}; };

    UnitCell make_unit_cell(CrystalStructure structure, Orientation orientation) {
      ...
      switch (structure) {
        case CrystalStructure::BCC: {
          switch (orientation) {
            case _001: case _010: case _100:
              uc.atoms = regular_bcc();
              uc.dims = {a_bcc, a_bcc, a_bcc};
              scale_positions(uc.atoms, uc.dims);
              break;
            case _110: case _101: case _011:
              uc.atoms = regular_fcc();
              uc.dims = {a_bcc, a_bcc, a_bcc};
              if (orientation == _110) { uc.dims[0] *= std::sqrt(2.0); uc.dims[1] *= std::sqrt(2.0); }
              if (orientation == _101) { uc.dims[0] *= std::sqrt(2.0); uc.dims[2] *= std::sqrt(2.0); }
              if (orientation == _011) { uc.dims[1] *= std::sqrt(2.0); uc.dims[2] *= std::sqrt(2.0); }
              scale_positions(uc.atoms, uc.dims);
              break;
            case _111:
              uc.atoms = {
        ...
        case CrystalStructure::FCC: {
          switch (orientation) {
            ...
            case _110: case _101: case _011:
              uc.atoms = regular_bcc();
              uc.dims = {1.0, 1.0, 1.0};
              if (orientation == _110) uc.dims[2] *= std::sqrt(2.0);
              if (orientation == _101) uc.dims[1] *= std::sqrt(2.0);
              if (orientation == _011) uc.dims[0] *= std::sqrt(2.0);
              scale_positions(uc.atoms, uc.dims);
              break;
            case _111:
              uc.atoms = {
  ...
  Lattice::Lattice(CrystalStructure structure, Orientation orientation, std::vector<long> shape)
      : structure_(structure), orientation_(orientation), shape_(std::move(shape)) {
    if (shape_.size() != 3) throw std::invalid_argument("Shape must have exactly 3 elements");
  ...
  arma::mat Lattice::positions(double nearest_neighbor_distance) const {
    return positions_ * nearest_neighbor_distance;
  }

  arma::mat Lattice::positions(const Vector3& box) const {
    arma::mat result = positions_;
    result.each_row() %= box / dimensions_;
    return result;
  }

  void Lattice::export_to(const std::string& filename, FileFormat format) const {
