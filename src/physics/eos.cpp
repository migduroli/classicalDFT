...
namespace dft::physics::eos {
  ...
  template <typename T> auto LennardJonesJZG::g_integral(T rho, int i) const -> T {
    using std::exp;
  ...
  template <typename T> auto LennardJonesJZG::excess_free_energy(T density) const -> T {
    using autodiff::detail::pow;
    using std::pow;
    T f = T(0.0);
    for (int i = 1; i <= 8; ++i) {
  ...
  template <typename T> auto LennardJonesMecke::excess_free_energy(T density) const -> T {
    using autodiff::detail::pow;
    using std::exp;
    using std::pow;
