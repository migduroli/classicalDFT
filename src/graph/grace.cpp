  ...
  namespace grace_plot {
    namespace command {
      ...
      std::string set_legend_command(const std::string& legend, const int& dataset_id, const int& graph_id) {
        std::string cmd =
            "G" + std::to_string(graph_id) + ".S" + std::to_string(dataset_id) + " LEGEND \"" + legend + "\"";
        return cmd;
      }
      ...
      std::string set_symbol_size_command(const double& size, const int& dataset_id, const int& graph_id) {
        ...
        std::string cmd = "G" + std::to_string(graph_id) + ".S" + std::to_string(dataset_id) + " SYMBOL SIZE " +
                          std::to_string(size_f);
        return cmd;
      }
...
namespace dft_core {
  namespace grace_plot {
    ...
    void start_grace_communication(const int& x_size, const int& y_size, int buffer_size) {
      ...
      auto response = GraceOpenVA(grace_name, buffer_size, option::FREE.c_str(), option::NO_SAFE.c_str(),
                                  option::GEOMETRY.c_str(), geometry_spec.c_str(), NULL);
