...
namespace dft::plotting::command {
  ...
  std::string set_format_command(const ExportFormat& format) {
    ...
    if (ExportFormat::PNG == format) {
      ...
      dft::console::warning("The format specified is not yet implemented. Exporting as PNG...");
      format_s = "PNG";
    }
...
namespace dft::plotting {
  ...
  void send_command(const std::string& cmd) {
    if (GraceIsOpen()) {
      try {
        ...
        throw dft::exception::GraceCommunicationFailedException();
      }
    } else {
      throw dft::exception::GraceNotOpenedException();
    }
  }
  ...
  void register_grace_error_function() {
    try {
      ...
      throw dft::exception::GraceException("error_parsing_function could not be registered by xmgrace");
    }
  }

  void start_grace_communication(const int& x_size, const int& y_size, int buffer_size) {
    if ((x_size <= 0) || (y_size <= 0)) {
      throw dft::exception::GraceException("The xmgrace-canvas dimensions cannot be negative!");
    }
    if (buffer_size <= 0) {
      throw dft::exception::GraceException("The communication buffer size cannot be negative!");
    }
  ...
  int get_number_of_rows(const int& number_of_graphs) {
    if (number_of_graphs < 1) {
      throw dft::exception::GraceException("Number of graphs cannot be lesser than one");
    }
  ...
  int get_number_of_columns(const int& number_of_graphs, const int& number_of_rows) {
    if (number_of_graphs < 1) {
      ...
      throw dft::exception::GraceException("Number of rows cannot be lesser than one");
    }
