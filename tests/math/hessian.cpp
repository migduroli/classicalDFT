...
TEST_CASE("HessianOperator stores dimension", "[hessian]") {
  HessianOperator op{
      .hessian_dot_v = [](const arma::vec&, arma::vec&) {},
      .dimension = 10,
  };
...
TEST_CASE("HessianOperator applies identity", "[hessian]") {
  HessianOperator op{
      .hessian_dot_v = [](const arma::vec& v, arma::vec& out) { out = v; },
      .dimension = 3,
  };
  arma::vec x = {1.0, 2.0, 3.0};
  arma::vec result(3);
  op.hessian_dot_v(x, result);
...
TEST_CASE("HessianOperator applies scaling", "[hessian]") {
  HessianOperator op{
      .hessian_dot_v = [](const arma::vec& v, arma::vec& out) { out = 2.0 * v; },
      .dimension = 2,
  };
  arma::vec x = {3.0, 4.0};
  arma::vec result(2);
...
TEST_CASE("HessianOperator applies matrix-vector product", "[hessian]") {
  ...
  HessianOperator op{
      .hessian_dot_v = [&H](const arma::vec& v, arma::vec& out) { out = H * v; },
      .dimension = 2,
  };
  arma::vec x = {1.0, 1.0};
  arma::vec result(2);
