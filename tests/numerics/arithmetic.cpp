...
using summation::CompensatedSum;
using summation::error_type;
using summation::return_type;
using summation::Type;
using summation::x_type;
...
TEST_P(KahanBabuskaTest, SumProducesCorrectResult) {
  ...
  auto [sum, err] = summation::kahan_babuska_sum(input);
  EXPECT_DOUBLE_EQ(expected, sum);
}

TEST_P(KahanBabuskaNeumaierTest, SumProducesCorrectResult) {
  auto [name, input, expected] = GetParam();
  auto [sum, err] = summation::kahan_babuska_neumaier_sum(input);
  EXPECT_DOUBLE_EQ(expected, sum);
}

TEST_P(KahanBabuskaKleinTest, SumProducesCorrectResult) {
  auto [name, input, expected] = GetParam();
  auto [sum, err] = summation::kahan_babuska_klein_sum(input);
  EXPECT_DOUBLE_EQ(expected, sum);
}
...
TEST(KahanBabuska, ContinuedSummation) {
  ...
  auto [sum1, err1] = summation::kahan_babuska_sum(part1);
  auto part2 = std::vector<double>{3.0, 4.0};
  auto [sum2, err2] = summation::kahan_babuska_sum(part2, sum1, err1);
  EXPECT_DOUBLE_EQ(10.0, sum2);
}

TEST(KahanBabuskaNeumaier, ContinuedSummation) {
  auto part1 = std::vector<double>{1.0, 2.0};
  auto [sum1, err1] = summation::kahan_babuska_neumaier_sum(part1);
  auto part2 = std::vector<double>{3.0, 4.0};
  auto [sum2, err2] = summation::kahan_babuska_neumaier_sum(part2, sum1, err1);
  EXPECT_DOUBLE_EQ(10.0, sum2);
}

TEST(KahanBabuskaKlein, ContinuedSummation) {
  auto part1 = std::vector<double>{1.0, 2.0};
  auto [sum1, err1] = summation::kahan_babuska_klein_sum(part1);
  auto part2 = std::vector<double>{3.0, 4.0};
  auto [sum2, err2] = summation::kahan_babuska_klein_sum(part2, sum1, err1);
  EXPECT_DOUBLE_EQ(10.0, sum2);
}
...
TEST(StandardVectorSum, ComputesCorrectSum) {
  ...
  EXPECT_DOUBLE_EQ(summation::standard_vector_sum(input), 10.0);
}

TEST(StandardVectorSum, EmptyVector) {
  auto input = std::vector<double>{};
  EXPECT_DOUBLE_EQ(summation::standard_vector_sum(input), 0.0);
}

TEST(StandardVectorSum, FloatVersion) {
  auto input = std::vector<float>{1.0f, 2.0f, 3.0f};
  EXPECT_FLOAT_EQ(summation::standard_vector_sum(input), 6.0f);
}
