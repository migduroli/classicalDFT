// Cross-validation of interaction weights against legacy code.
  ...
  for (int ix = 0; ix <= lim + 1; ++ix)
    for (int iy = 0; iy <= ix; ++iy)
      for (int iz = 0; iz <= iy; ++iz) {
        ...
        double ref_val = legacy::interactions::generate_weight_QF(SIGMA, EPS, RCUT, ix, iy, iz, dx) / KT;
        CHECK(ours == Approx(ref_val).margin(1e-10));
      }
}
...
TEST_CASE("Grid a_vdw matches legacy at multiple dx", "[integration][interaction]") {
  ...
  for (double dx : {0.5, 0.4, 0.3, 0.2}) {
    double ref_a = legacy::interactions::compute_a_vdw_QF(SIGMA, EPS, RCUT, KT, dx);
    ...
    INFO("dx=" << dx);
    CHECK(our_sum == Approx(ref_a).margin(1e-10));
  }
}
