// Cross-validation of spinodal/coexistence solvers against legacy code.
...
TEST_CASE("Spinodal matches legacy at kT=0.7", "[integration][solver]") {
  ...
  auto ref = legacy::solver::findSpinodal(fix.legacy_eos, 1.0, 0.005);
  REQUIRE(ours.has_value());
  CHECK(ours->rho_low == Approx(ref.xs1).epsilon(1e-6));
  CHECK(ours->rho_high == Approx(ref.xs2).epsilon(1e-6));
}
...
TEST_CASE("Coexistence matches legacy at kT=0.7", "[integration][solver]") {
  ...
  auto ref = legacy::solver::findCoex(fix.legacy_eos, 1.0, 0.005);
  REQUIRE(ours.has_value());
  CHECK(ours->rho_vapor == Approx(ref.x1).epsilon(1e-6));
  CHECK(ours->rho_liquid == Approx(ref.x2).epsilon(1e-6));
}
