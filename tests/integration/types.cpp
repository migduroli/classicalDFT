// Cross-validation of crystal lattice positions against legacy code.
...
static void compare_lattices(Structure structure, const std::string& orient, int nx = 2, int ny = 2, int nz = 2) {
  ...
  // Legacy code
  auto ref_lat = legacy::crystal::build(lattice_name, orient, nx, ny, nz);

  REQUIRE(ours.n_rows == ref_lat.atoms.size());

  // Convert legacy atoms to arma::mat for sorting.
  arma::mat ref_mat(ref_lat.atoms.size(), 3);
  for (size_t i = 0; i < ref_lat.atoms.size(); ++i) {
    ref_mat(i, 0) = ref_lat.atoms[i][0];
    ref_mat(i, 1) = ref_lat.atoms[i][1];
    ref_mat(i, 2) = ref_lat.atoms[i][2];
  }
  auto ref_sorted = sorted_positions(ref_mat);

  for (arma::uword i = 0; i < ours.n_rows; ++i) {
    INFO("atom " << i << " in " << lattice_name << " " << orient);
    CHECK(ours(i, 0) == Approx(ref_sorted(i, 0)).margin(1e-12));
    CHECK(ours(i, 1) == Approx(ref_sorted(i, 1)).margin(1e-12));
    CHECK(ours(i, 2) == Approx(ref_sorted(i, 2)).margin(1e-12));
  }

  // Box dimensions should also match.
  CHECK(lattice.dimensions(0) == Approx(ref_lat.L[0]).margin(1e-12));
  CHECK(lattice.dimensions(1) == Approx(ref_lat.L[1]).margin(1e-12));
  CHECK(lattice.dimensions(2) == Approx(ref_lat.L[2]).margin(1e-12));
}
...
TEST_CASE("Scaled BCC positions match legacy", "[integration][crystal]") {
  ...
  auto ref_lat = legacy::crystal::build("BCC", "001", 2, 2, 2);
  auto ref_scaled = legacy::crystal::scaled(ref_lat, dnn);
  arma::mat ref_mat(ref_scaled.atoms.size(), 3);
  for (size_t i = 0; i < ref_scaled.atoms.size(); ++i) {
    ref_mat(i, 0) = ref_scaled.atoms[i][0];
    ref_mat(i, 1) = ref_scaled.atoms[i][1];
    ref_mat(i, 2) = ref_scaled.atoms[i][2];
  }
  auto ref_sorted = sorted_positions(ref_mat);

  REQUIRE(ours.n_rows == ref_sorted.n_rows);
  for (arma::uword i = 0; i < ours.n_rows; ++i) {
    CHECK(ours(i, 0) == Approx(ref_sorted(i, 0)).margin(1e-12));
    CHECK(ours(i, 1) == Approx(ref_sorted(i, 1)).margin(1e-12));
    CHECK(ours(i, 2) == Approx(ref_sorted(i, 2)).margin(1e-12));
