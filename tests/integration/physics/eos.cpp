// Cross-validation of hard-sphere and LJ EOS against legacy code.
...
TEST_CASE("LJ JZG coefficients match legacy", "[integration][thermodynamics]") {
  for (double kT : {0.7, 1.0, 1.5, 2.0}) {
    ...
    auto ref_jzg = legacy::thermodynamics::make_LJ_JZG(kT, 2.5);
    for (int i = 1; i <= 8; ++i)
      CHECK(our_jzg.a_coeff(i) == Approx(ref_jzg.a(i)).margin(1e-10));
    for (int i = 1; i <= 6; ++i)
      CHECK(our_jzg.b_coeff(i) == Approx(ref_jzg.b(i)).margin(1e-10));
    for (double rho : {0.1, 0.4, 0.8})
      for (int i = 1; i <= 6; ++i)
        CHECK(our_jzg.g_integral(rho, i) == Approx(ref_jzg.G(rho, i)).margin(1e-10));
  }
}
...
TEST_CASE("JZG phix and derivatives match legacy", "[integration][thermodynamics]") {
  for (double kT : {0.7, 1.0, 1.5, 2.0}) {
    ...
    for (double rho : {0.1, 0.2, 0.4, 0.6, 0.8}) {
      CHECK(our_jzg.excess_free_energy(rho) == Approx(ref_jzg.phix(rho)).margin(1e-10));
      CHECK(our_jzg.d_excess_free_energy(rho) == Approx(ref_jzg.phi1x(rho)).epsilon(1e-7));
      CHECK(our_jzg.d2_excess_free_energy(rho) == Approx(ref_jzg.phi2x(rho)).epsilon(1e-4));
    }
  }
}
...
TEST_CASE("Mecke phix and phi1x match legacy", "[integration][thermodynamics]") {
  for (double kT : {0.7, 1.0, 1.5, 2.0}) {
    ...
    auto ref_mecke = legacy::thermodynamics::make_LJ_Mecke(kT, 2.5);
    for (double rho : {0.1, 0.2, 0.4, 0.6, 0.8}) {
      CHECK(our_mecke.excess_free_energy(rho) == Approx(ref_mecke.phix(rho)).margin(1e-10));
      CHECK(our_mecke.d_excess_free_energy(rho) == Approx(ref_mecke.phi1x(rho)).epsilon(1e-7));
    }
  }
}
