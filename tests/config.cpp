...
TEST(ConfigParser, DefaultConstructorUsesIniFile) {
  ...
  ASSERT_EQ(config.config_file_type(), config::FileType::INI);
}
...
TEST_P(ConfigParserFileTest, ConstructsAndReadsCorrectly) {
  ...
  auto config = config::ConfigParser(file, type);
...
INSTANTIATE_TEST_SUITE_P(
    ...
    testing::Values(
        ConfigFileTestCase{"INI", "config.ini", config::FileType::INI},
        ConfigFileTestCase{"JSON", "config.json", config::FileType::JSON}
    ),
...
TEST(ConfigParser, GetReturnsCorrectValues) {
  auto config = config::ConfigParser("config.ini");
...
TEST(ConfigParser, DataAccessorReturnsJson) {
  auto config = config::ConfigParser("config.json", config::FileType::JSON);
...
TEST(ConfigParser, ThrowsOnMissingIniFile) {
  EXPECT_THROW(config::ConfigParser("nonexistent.ini", config::FileType::INI), std::runtime_error);
}

TEST(ConfigParser, ThrowsOnMissingJsonFile) {
  EXPECT_THROW(config::ConfigParser("nonexistent.json", config::FileType::JSON), std::runtime_error);
}
...
TEST_F(IniEdgeCaseTest, CommentsAreSkipped) {
  auto config = config::ConfigParser("edge_case.ini", config::FileType::INI);
...
TEST(ConfigParser, MalformedIniSectionThrows) {
  ...
  f << "key = value\n";
  f.close();
  EXPECT_THROW(config::ConfigParser("malformed.ini", config::FileType::INI), std::runtime_error);
...
TEST(ConfigParser, IniLineWithoutEqualsIsSkipped) {
  ...
  f << "key = value\n";
  f.close();
  auto config = config::ConfigParser("bare_line.ini", config::FileType::INI);
  EXPECT_EQ(config.get<std::string>("section.key"), "value");
...
TEST(ConfigParser, SetConfigFileType) {
  auto config = config::ConfigParser("config.ini");
  config.set_config_file_type(config::FileType::JSON);
  EXPECT_EQ(config.config_file_type(), config::FileType::JSON);
}

TEST(ConfigParser, ReadConfigFileReloads) {
  auto config = config::ConfigParser("config.ini");
  config.read_config_file("config.json", config::FileType::JSON);
