...
TEST_CASE("StepProfile applies step function to distances", "[fields]") {
  ...
  auto field = StepProfile{.radius = 2.0, .rho_in = 1.0, .rho_out = 0.1}(distances);
...
TEST_CASE("StepProfile with zero radius gives all rho_out", "[fields]") {
  ...
  auto field = StepProfile{.radius = 0.0, .rho_in = 5.0, .rho_out = 0.5}(distances);
...
TEST_CASE("EllipsoidalEnvelope is 1 at center and 0 far away", "[fields]") {
  ...
  auto envelope = EllipsoidalEnvelope{.radii = {3.0, 3.0, 3.0}, .interface_width = 0.25}(grid, center);
...
TEST_CASE("EllipsoidalEnvelope respects non-periodic axis", "[fields]") {
  ...
  auto envelope = EllipsoidalEnvelope{.radii = {2.0, 2.0, 2.0}, .interface_width = 0.25}(grid, center);
...
TEST_CASE("EllipsoidalEnvelope handles ellipsoidal radii", "[fields]") {
  ...
  auto envelope = EllipsoidalEnvelope{.radii = {4.0, 2.0, 3.0}, .interface_width = 0.25}(grid, center);
