...
TEST_CASE("ideal gas free energy for uniform density", "[functionals][ideal_gas]") {
  ...
  // beta*F_id = rho*(ln(rho)-1) * V (dimensionless, independent of kT).
  double expected = rho * (std::log(rho) - 1.0) * dv * grid.total_points();
...
TEST_CASE("ideal gas force includes chemical potential", "[functionals][ideal_gas]") {
  ...
  auto result = ideal_gas(grid, state);
  double dv = grid.cell_volume();
  // chemical_potential is in beta*mu units, so no division by kT.
  double expected_force = (std::log(rho) - mu) * dv;
  CHECK(result.forces[0](0) == Catch::Approx(expected_force).margin(1e-14));
}
...
TEST_CASE("ideal gas free energy is temperature-independent", "[functionals][ideal_gas]") {
  ...
  // beta*F_id = rho*(ln(rho)-1)*V does not depend on temperature.
  auto r1 = ideal_gas(grid, state1);
  auto r2 = ideal_gas(grid, state2);
  CHECK(r2.free_energy == Catch::Approx(r1.free_energy).margin(1e-10));
}
