...
namespace dft::functional::interaction {
  ...
  const species::Species& Interaction::species_1() const noexcept {
    return s1_;
  }

  const species::Species& Interaction::species_2() const noexcept {
    return s2_;
  }
  ...
  double Interaction::compute_cell_weight_gauss(long sx, long sy, long sz) const {
    ...
    auto table = std::unique_ptr<gsl_integration_glfixed_table, decltype(&gsl_integration_glfixed_table_free)>(
        gsl_integration_glfixed_table_alloc(static_cast<size_t>(gauss_order_)), gsl_integration_glfixed_table_free
    );
    ...
    for (int i = 0; i < gauss_order_; ++i) {
      double xi = 0.0;
      double wi = 0.0;
      gsl_integration_glfixed_point(-0.5, 0.5, static_cast<size_t>(i), &xi, &wi, table.get());
      double x = (static_cast<double>(sx) + xi) * dx_;

      for (int j = 0; j < gauss_order_; ++j) {
        double xj = 0.0;
        double wj = 0.0;
        gsl_integration_glfixed_point(-0.5, 0.5, static_cast<size_t>(j), &xj, &wj, table.get());
        double y = (static_cast<double>(sy) + xj) * dx_;

        for (int k = 0; k < gauss_order_; ++k) {
          double xk = 0.0;
          double wk = 0.0;
          gsl_integration_glfixed_point(-0.5, 0.5, static_cast<size_t>(k), &xk, &wk, table.get());
