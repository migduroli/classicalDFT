...
#include <cmath>
using namespace dft::math::integration;

namespace dft::potentials {
  ...
  double Potential::sigma() const {
    return sigma_;
  }
  double Potential::epsilon() const {
    return epsilon_;
  }
  double Potential::epsilon_shift() const {
    return epsilon_shift_;
  }
  double Potential::r_cutoff() const {
    return r_cutoff_;
  }
  double Potential::r_min() const {
    return r_min_;
  }
  double Potential::v_min() const {
    return v_min_;
  }
  double Potential::r_attractive_min() const {
    return r_attractive_min_;
  }
  double Potential::r_zero() const {
    return r_zero_;
  }
  bool Potential::bh_perturbation() const {
    return bh_perturbation_;
  }
  double Potential::kT() const {
    return kT_;
  }
  const PotentialName& Potential::id() const {
    return potential_id_;
  }
  ...
  double Potential::_v_potential(double r) const {
    return vr_(r) - epsilon_shift();
  }

  double Potential::v_potential(double r) const {
  ...
  double Potential::_w_attractive(double r) const {
    return this->w_attractive_r2(r * r);
  }

  double Potential::w_attractive(double r) const {
  ...
  double Potential::vdw_kernel(double r) const {
    return r * r * w_attractive(r);
  }

  double Potential::find_hard_sphere_diameter(double kT) {
    kT_ = kT;
    auto d_hard_core = find_hard_core_diameter();
    auto r_limit = bh_perturbation_ ? r_zero() : r_min();
    auto integrator = Integrator<Potential>(*this, &Potential::bh_diameter_kernel, 1e-4, 1e-6);
    auto d_hard_sphere = d_hard_core + integrator.definite_integral(d_hard_core, r_limit);
    return d_hard_sphere;
  }
  ...
  double Potential::compute_van_der_waals_integral(double kT) {
    ...
    auto integral = bh_perturbation() ? integrator.definite_integral(r_zero(), limit_superior)
  ...
  arma::vec Potential::operator()(const arma::vec& r) const {
    return v_potential(r);
  }
  ...
  LennardJones::LennardJones(double sigma, double epsilon, double r_cutoff) : Potential(sigma, epsilon, r_cutoff) {
    potential_id_ = PotentialName::LennardJones;
    epsilon_shift_ = (r_cutoff_ < 0 ? 0.0 : this->vr_(r_cutoff_));
    r_min_ = this->find_r_min();
    v_min_ = this->v_potential(r_min_);
  ...
  double LennardJones::find_r_min() const {
    return std::pow(2.0, 1.0 / 6.0) * sigma_;
  }

  double LennardJones::find_hard_core_diameter() const {
    return 0;
  }
  ...
  double tenWoldeFrenkel::alpha() const {
    return alpha_;
  }
  ...
  double tenWoldeFrenkel::find_hard_core_diameter() const {
    return sigma_;
  }
  ...
  WangRamirezDobnikarFrenkel::WangRamirezDobnikarFrenkel(double sigma, double epsilon, double r_cutoff)
    ...
    // The cutoff is built in:
    epsilon_ *=
        2 * std::pow(r_cutoff / sigma, 2) * std::pow(2 * ((r_cutoff / sigma) * (r_cutoff / sigma) - 1) / 3.0, -3.0);
    epsilon_shift_ = 0.0;
  ...
  double WangRamirezDobnikarFrenkel::find_hard_core_diameter() const {
    return 0;
  }
