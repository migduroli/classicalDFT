#ifndef DFT_PLOTTING_EXCEPTIONS_H
#define DFT_PLOTTING_EXCEPTIONS_H
...
namespace dft::exception {
   ...
   *
   * This class will serve as the base class for the exceptions we want to register regarding undesired or
   * uncontrolled behaviours when dealing with `xmgrace`
   */
  ...
  /**
   * @brief Exception to be thrown when detecting that `xmgrace` communication is not opened (`GraceIsOpen() ==
   * false`)
   *
