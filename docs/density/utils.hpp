...
namespace utils {
  ...
  [[nodiscard]] inline auto extract_profile(
      const arma::vec& rho_3d, long nx, long ny, long nz
  ) -> std::vector<double> {
  ...
  [[nodiscard]] inline auto make_state(
      const dft::physics::Model& model, const arma::vec& rho, double mu
  ) -> dft::State {
  ...
  [[nodiscard]] inline auto make_force_fn(
      const dft::physics::Model& model,
      const dft::functionals::Weights& weights,
      double mu
  ) {
    return [&model, &weights, mu](const std::vector<arma::vec>& densities)
