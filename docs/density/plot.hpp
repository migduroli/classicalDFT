...
namespace plot {
  ...
  namespace detail {
    ...
    inline auto spline_refine(const std::vector<double>& x, const std::vector<double>& y, int n_fine = fine_grid_points)
    ...
    inline void free_energy(
      ...
      namespace plt = matplotlibcpp;
      plt::figure_size(900, 600);
      plt::plot(rho, f, {{"color", "k"}, {"linestyle", "-"}, {"linewidth", "1.5"}, {"label", R"($f(\rho)$)"}});
      plt::plot(
          {rho_v, rho_l},
          {f_v, f_l},
          {{"color", "#d62728"}, {"linestyle", "--"}, {"linewidth", "1.2"}, {"label", "Common tangent"}}
      );
    ...
    inline void chemical_potential(
      ...
      namespace plt = matplotlibcpp;
      plt::figure_size(900, 600);
      plt::plot(rho, mu, {{"color", "k"}, {"linestyle", "-"}, {"linewidth", "1.5"}, {"label", R"($\mu(\rho)$)"}});
      plt::plot(
          {0.0, 1.0},
          {mu_coex, mu_coex},
          {{"color", "#d62728"}, {"linestyle", "--"}, {"linewidth", "1.2"}, {"label", R"($\mu_\mathrm{coex}$)"}}
      );
    ...
    inline void density_evolution(
      ...
      plt::plot(
          xi,
          yi,
          {{"color", "#00000044"}, {"linestyle", "--"}, {"linewidth", "1.0"}, {"label", "Initial (tanh)"}}
      );
      ...
      for (std::size_t i = 1; i < snapshots.size(); ++i) {
        ...
        plt::plot(xs, ys, {{"color", colors[(i - 1) % colors.size()]}, {"linestyle", "-"}, {"label", label}});
      }

      // Final relaxed profile (thick, spline-interpolated).
      auto [xf, yf] = spline_refine(x, final_profile);
      plt::plot(xf, yf, {{"color", "#1f77b4"}, {"linestyle", "-"}, {"linewidth", "2.0"}, {"label", "Relaxed"}});

      // Coexistence reference.
      plt::plot({x.front(), x.back()}, {rho_v, rho_v}, {{"color", "#d6272833"}, {"linestyle", ":"}});
      plt::plot({x.front(), x.back()}, {rho_l, rho_l}, {{"color", "#2ca02c33"}, {"linestyle", ":"}});
    ...
    inline void grand_potential(const std::vector<double>& times, const std::vector<double>& omega) {
      namespace plt = matplotlibcpp;
      plt::figure_size(900, 600);
      plt::plot(
          times,
  ...
  inline void make_plots(
      const std::vector<double>& rho_range,
      const std::vector<double>& p_range,
      const std::vector<double>& f_range,
      const std::vector<double>& mu_range,
      double rho_v,
      double rho_l,
      double p_coex,
      double f_v,
