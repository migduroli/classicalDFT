...
Every quantity is cross-validated against Jim Lutsko's original classicalDFT
...
The `check/main.cpp` program compares every computed quantity against Jim
Lutsko's classicalDFT implementation (`Potential1.h`, `Potential.cpp`).
...
quadrature while Jim's code uses midpoint-rule with $10^6$ points. Both
