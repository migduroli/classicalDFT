...
namespace plot {
  ...
  namespace detail {
    ...
    inline void convergence(const std::vector<double>& iterations, const std::vector<double>& residuals) {
    ...
    inline void density_profile(
      ...
      plt::plot(x, rho_converged, {{"color", "#008080"}, {"linewidth", "2.0"}, {"label", R"($\rho^*$ converged)"}});
