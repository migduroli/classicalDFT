...
int main() {
  ...
  for (const auto& cfg : configs) {
    ...
    std::println(std::cout, "{:>14s}{:>10d}{:>12.6f}{:>12.6f}{:>12.6f}",
                 cfg.label, lattice.positions.n_rows,
                 lattice.dimensions(0), lattice.dimensions(1), lattice.dimensions(2));
  }
  ...
  std::println(std::cout, "  Dimensions: ({}, {}, {})",
               fcc.dimensions(0), fcc.dimensions(1), fcc.dimensions(2));
  ...
  for (arma::uword i = 0; i < std::min<arma::uword>(bcc.positions.n_rows, 4); ++i) {
    std::println(std::cout, "    atom {}: ({:.6f}, {:.6f}, {:.6f})",
                 i, scaled(i, 0), scaled(i, 1), scaled(i, 2));
  }
  ...
  for (arma::uword i = 0; i < std::min<arma::uword>(bcc.positions.n_rows, 4); ++i) {
    std::println(std::cout, "    atom {}: ({:.6f}, {:.6f}, {:.6f})",
                 i, aniso(i, 0), aniso(i, 1), aniso(i, 2));
  }
