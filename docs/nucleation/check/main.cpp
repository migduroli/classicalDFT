...
int main() {
  ...
  double S_cfg = cfg["droplet"].contains("supersaturation") ? cfg["droplet"]["supersaturation"].get<double>() : 1.1;
  ...
  std::cout << "    rho_in=" << rho_in << " S=" << S_cfg << " R=" << R0 << "\n\n";
  ...
  double hsd_ours = our_lj.hard_sphere_diameter(kT, physics::potentials::SplitScheme::WeeksChandlerAndersen);
  ...
  double vdw_ours = our_lj.vdw_integral(kT, physics::potentials::SplitScheme::WeeksChandlerAndersen);
  ...
  double rho_v = coex ? coex->rho_vapor : 0.001;
  double rho_l = coex ? coex->rho_liquid : rho_in;
  double rho_out = S_cfg * rho_v;
  double mu_rho_out = eos.chemical_potential(arma::vec{rho_out}, 0);
  std::cout << "  Supersaturation S = " << S_cfg << "  rho_out = " << rho_out << "\n";
  std::cout << "  mu(rho_out) = " << mu_rho_out << "\n\n";
  ...
  auto cluster =
      algorithms::minimization::Minimizer{
          ...
          .fixed_mass(model, weights, rho0, mu_rho_out, target_mass);
