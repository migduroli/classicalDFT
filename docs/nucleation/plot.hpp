...
namespace plot {
  ...
  namespace detail {
    ...
    [[nodiscard]] inline auto filter_pathway(
        const std::vector<nucleation::PathwayPoint>& pts, double rho_bg
    ) -> std::vector<nucleation::PathwayPoint> {
      std::vector<nucleation::PathwayPoint> out;
      double thr = rho_bg * 1.5;
      for (const auto& p : pts) {
        if (p.rho_center > thr && p.radius > 0.0) out.push_back(p);
      }
      return out;
    }
    ...
    inline void plot_energy_barrier(
      ...
      plt::plot(r_d, de_d, {{"color", "#00BBD5"}, {"linewidth", "1.5"}, {"marker", "o"},
                            {"markersize", "4"}, {"label", R"(Dissolution ($N < N^*$))"}});
      plt::plot(r_g, de_g, {{"color", "#E25822"}, {"linewidth", "1.5"}, {"marker", "o"},
                            {"markersize", "4"}, {"label", R"(Growth ($N > N^*$))"}});

      plt::plot({critical.radius}, {delta_omega_critical},
      ...
      for (double e : de_d) { e_min_data = std::min(e_min_data, e); e_max_data = std::max(e_max_data, e); }
      for (double e : de_g) { e_min_data = std::min(e_min_data, e); e_max_data = std::max(e_max_data, e); }

      double r_pad = std::max(0.2 * (r_max_data - r_min_data), 0.5);
      double e_pad = std::max(0.1 * (e_max_data - e_min_data), 1.0);

      plt::plot({r_min_data - r_pad, r_max_data + r_pad}, {0.0, 0.0},
    ...
    inline void plot_rho_center_vs_radius(
      ...
      plt::plot({0.0, r_max_data + r_pad}, {rho_v, rho_v},
                {{"color", "#00976E"}, {"linestyle", "--"}, {"linewidth", "1.0"},
                 {"label", R"($\rho_v(\mu)$)"}});
      plt::plot({0.0, r_max_data + r_pad}, {rho_l, rho_l},
                {{"color", "#00976E"}, {"linestyle", "--"}, {"linewidth", "1.0"},
                 {"label", R"($\rho_l(\mu)$)"}});
  ...
  inline void make_plots(
    ...
    detail::plot_dynamics(
        ...
        0, 187, 213, 0, 62, 62,
        export_dir + "/dissolution.png", model_name);

    detail::plot_dynamics(
        growth.profiles, critical_profile, rho_v, rho_l,
        R"(Growth dynamics ($N > N^*$))",
        226, 88, 34, 62, 0, 0,
        export_dir + "/growth.png", model_name);
