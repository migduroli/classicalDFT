...
namespace plot {
  ...
  namespace detail {
    ...
    inline void spline_interpolation(
        ...
        const std::vector<double>& y_spline,
        const std::vector<double>& y_exact
    ) {
      namespace plt = matplotlibcpp;
      plt::figure_size(800, 500);

      plt::plot(x_eval, y_exact, { { "color", "#656775" }, { "linewidth", "2.0" }, { "label", R"($\sin(x)$ exact)" } });
      plt::plot(
          x_eval,
          y_spline,
    ...
    inline void spline_derivatives(
      ...
      plt::plot(x_eval, y_exact_func, { { "color", "#656775" }, { "linewidth", "2.0" }, { "label", R"($\sin(x)$)" } });
      plt::plot(
          x_eval,
          y_func,
      ...
      plt::plot(x_eval, y_exact_d1, { { "color", "#00BBD5" }, { "linewidth", "2.0" }, { "label", R"($\cos(x)$)" } });
      plt::plot(
          x_eval,
          y_deriv1,
      ...
      plt::plot(x_eval, y_exact_d2, { { "color", "#E25822" }, { "linewidth", "2.0" }, { "label", R"($-\sin(x)$)" } });
      plt::plot(
          x_eval,
          y_deriv2,
