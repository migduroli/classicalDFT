...
int main() {
  ...
  auto [sv, sd] = derivatives_up_to_1([](dual x) -> dual { return autodiff::detail::sin(x); }, std::numbers::pi / 4.0);
  double sd_exact = std::cos(std::numbers::pi / 4.0);
  std::println(
      std::cout,
      "  {:>14s}{:>8s}{:>18.12f}{:>18.12f}{:>18.12f}{:>14.4e}",
      "sin(x)",
      "pi/4",
      sv,
      sd,
  ...
  auto [ev, ed] = derivatives_up_to_1([](dual x) -> dual { return autodiff::detail::exp(x); }, 1.0);
  std::println(
      std::cout,
      "  {:>14s}{:>8s}{:>18.12f}{:>18.12f}{:>18.12f}{:>14.4e}",
      "exp(x)",
      ...
      ed,
      std::exp(1.0),
      std::abs(ed - std::exp(1.0))
  );

  auto [pv, pd] = derivatives_up_to_1([](dual x) -> dual { return x * x * x - 2.0 * x * x + 3.0 * x - 1.0; }, 2.0);
  ...
  auto [s2v, s2d, s2d2] = derivatives_up_to_2([](dual2nd x) -> dual2nd { return autodiff::detail::sin(x); }, x0);
  ...
  auto [e2v, e2d, e2d2] = derivatives_up_to_2([](dual2nd x) -> dual2nd { return autodiff::detail::exp(x); }, 2.0);
  std::println(std::cout, "\n  exp(x) at x = 2:");
  ...
  auto [s3v, s3d, s3d2, s3d3] = derivatives_up_to_3([](dual3rd x) -> dual3rd { return autodiff::detail::sin(x); }, x0);
  ...
  auto [p3v, p3d, p3d2, p3d3] =
      derivatives_up_to_3([](dual3rd x) -> dual3rd { return x * x * x - 2.0 * x * x + 3.0 * x - 1.0; }, 2.0);
  ...
  auto [tv, td, td2] = derivatives_up_to_2([](dual2nd x) -> dual2nd { return autodiff::detail::log(1.0 + x * x); }, xc);

  double fd1 = (std::log(1.0 + (xc + h) * (xc + h)) - std::log(1.0 + (xc - h) * (xc - h))) / (2.0 * h);
  double fd2 = (std::log(1.0 + (xc + h) * (xc + h)) - 2.0 * std::log(1.0 + xc * xc)
                + std::log(1.0 + (xc - h) * (xc - h)))
      / (h * h);
  ...
  for (int i = 0; i < np_sin; ++i) {
    ...
    x_sin[i] = xi;
    auto [v, d, dd] = derivatives_up_to_2([](dual2nd t) -> dual2nd { return autodiff::detail::sin(t); }, xi);
    f_sin[i] = v;
    d1_sin[i] = d;
    d2_sin[i] = dd;
  }
  ...
  auto f_exact_fn = [](double x) {
    return std::log(1.0 + x * x);
  };
  auto d1_exact_fn = [](double x) {
    return 2.0 * x / (1.0 + x * x);
  };
  auto d2_exact_fn = [](double x) {
  ...
  for (int i = 0; i < np_err; ++i) {
    ...
    x_err[i] = xi;
    auto [v, d, dd] = derivatives_up_to_2([](dual2nd t) -> dual2nd { return autodiff::detail::log(1.0 + t * t); }, xi);
