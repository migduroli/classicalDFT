...
namespace plot {
  ...
  namespace detail {
    ...
    inline void coexistence(const std::vector<CoexData>& all_coex, const std::vector<SpinodalData>& all_spin = {}) {
      ...
      for (std::size_t m = 0; m < all_coex.size(); ++m) {
        ...
        if (m < all_spin.size() && !all_spin[m].T.is_empty()) {
          ...
          plt::plot(rho_sp_vec, T_sp_vec, {{"color", col_a}, {"linestyle", "--"}, {"label", sd.name + " (spinodal)"}});
        }
      }
      plt::xlabel(R"($\rho \sigma^3$)");
    ...
    inline void phase_diagram_plot(
        ...
        const utils::ScanCoexPoints& scan = {},
        const utils::ScanSpinodalPoints& scan_sp = {}
    ) {
      namespace plt = matplotlibcpp;
      plt::figure_size(900, 650);
      ...
      // Single-temperature coexistence points (scan + bisect).
      if (!scan.T.empty()) {
        plt::plot(
            scan.rho_v,
            scan.T,
      ...
      // Single-temperature spinodal points (scan + bisect).
      if (!scan_sp.T.empty()) {
        plt::plot(
            scan_sp.rho_lo,
            scan_sp.T,
  ...
  inline void make_plots(
      ...
      const utils::ScanCoexPoints& scan_pts = {},
      const utils::ScanSpinodalPoints& scan_sp = {}
  ) {
    ...
    if (!wb2_sp.temperature.is_empty()) {
      detail::phase_diagram_plot(wb2_data, wb2_sp, scan_pts, scan_sp);
    }
  }
