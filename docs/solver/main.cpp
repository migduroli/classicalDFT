...
int main() {
  ...
  // Cross-validate: compute single-temperature coexistence and
  // spinodal at many temperatures up to T_c, and overlay on the
  // continuation curves.

  double Tc_wb2 = wb2_data.Tc > 0 ? wb2_data.Tc : 1.3;
  std::println(std::cout, "\n=== Single-T coexistence & spinodal (scan + bisect) vs continuation ===\n");

  utils::ScanCoexPoints scan_pts;
  utils::ScanSpinodalPoints scan_sp;
  for (double kT = 0.60; kT <= Tc_wb2 + 0.02; kT += 0.01) {
    auto eos = wb2_eos_factory(kT);

    auto sp = config.find_spinodal(eos);
    if (sp) {
      scan_sp.T.push_back(kT);
      scan_sp.rho_lo.push_back(sp->rho_low);
      scan_sp.rho_hi.push_back(sp->rho_high);
    }

    auto coex = config.find_coexistence(eos);
    if (coex) {
      scan_pts.T.push_back(kT);
      scan_pts.rho_v.push_back(coex->rho_vapor);
      scan_pts.rho_l.push_back(coex->rho_liquid);
      std::println(
          std::cout,
  ...
  std::println(std::cout, "\n  Scan coexistence points: {}, spinodal points: {}", scan_pts.T.size(), scan_sp.T.size());

  // Collect spinodal data for plots.

  dft::functionals::bulk::SpinodalCurve
      wb2_sp{.temperature = wb2_spin.T, .rho_low = wb2_spin.rho_lo, .rho_high = wb2_spin.rho_hi};

#ifdef DFT_HAS_MATPLOTLIB
  plot::make_plots(iso_rho, iso_p, isotherm_temps, all_coex, all_spin, wb2_data, wb2_sp, scan_pts, scan_sp);
#endif
}
