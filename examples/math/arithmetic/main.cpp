...
int main()
{
  ...
  auto trivial_sum = standard_vector_sum(x_input_1);
  ...
  std::tie(kb_sum, kb_err) = kahan_babuska_sum(x_input_1);
  ...
  std::tie(neum_sum, neum_err) = kahan_babuska_neumaier_sum(x_input_1);
  ...
  std::tie(klein_sum, klein_err) = kahan_babuska_klein_sum(x_input_1);
  ...
  std::cout << std::endl;
  trivial_sum += standard_vector_sum(x_input_2);
  std::tie(kb_sum, kb_err) = kahan_babuska_sum(x_input_2, kb_sum, kb_err);
  std::tie(neum_sum, neum_err) = kahan_babuska_neumaier_sum(x_input_2, neum_sum, neum_err);
  std::tie(klein_sum, klein_err) = kahan_babuska_klein_sum(x_input_2, klein_sum, klein_err);
