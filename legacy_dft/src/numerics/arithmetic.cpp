...
namespace dft::numerics::arithmetic {
  ...
  const error_type& CompensatedSum::error() const {
    return this->error_;
  }
  const Type& CompensatedSum::type() const {
    return this->type_;
  }

  CompensatedSum::CompensatedSum(Type type) : type_(type) {
    if (type == Type::KahanBabuskaKlein) {
      error_ = std::vector<double>{0.0, 0.0};
      method_ = &kahan_babuska_klein_add;
    } else if (type == Type::KahanBabuska) {
      error_ = std::vector<double>{0.0};
      method_ = &kahan_babuska_add;
    } else if (type == Type::KahanBabuskaNeumaier) {
      error_ = std::vector<double>{0.0};
      method_ = &kahan_babuska_neumaier_add;
    } else {
      dft::io::console::warning("Method not implemented yer, defaulting to Kahan-Babuska-Neumaier");
      error_ = std::vector<double>{0.0};
      method_ = &kahan_babuska_neumaier_add;
    }
  }
  ...
  x_type CompensatedSum::sum() const {
    if (this->type_ == Type::KahanBabuska) {
      return this->sum_;
    }
  ...
  CompensatedSum& CompensatedSum::operator+=(const CompensatedSum& other) {
    sum_ += other.sum_;
    for (size_t k = 0; k < error_.size(); k++) {
  ...
  CompensatedSum& CompensatedSum::operator=(x_type x) {
    sum_ = x;
    error_ = (type_ == Type::KahanBabuskaKlein) ? error_type{0.0, 0.0} : error_type{0.0};
    return *this;
  }
