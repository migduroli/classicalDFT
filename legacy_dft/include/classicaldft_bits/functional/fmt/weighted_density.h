#ifndef CLASSICALDFT_FUNCTIONAL_FMT_WEIGHTED_DENSITY_H
#define CLASSICALDFT_FUNCTIONAL_FMT_WEIGHTED_DENSITY_H
...
namespace dft::functional::fmt {
  ...
  class WeightedDensity {
   public:
    ...
    WeightedDensity(WeightedDensity&&) noexcept = default;             // NOLINT(bugprone-exception-escape)
    WeightedDensity& operator=(WeightedDensity&&) noexcept = default;  // NOLINT(bugprone-exception-escape)
    ...
    [[nodiscard]] const numerics::fourier::FourierTransform& weight() const noexcept { return weight_; }

    /**
     * @brief Mutable access to the weight's Fourier coefficients.
     *
     * Used by WeightGenerator::generate() to populate the weights directly in k-space.
     */
   ...
   private:
    numerics::fourier::FourierTransform weight_;
    numerics::fourier::FourierTransform scratch_;
    arma::vec field_;
...
#endif  // CLASSICALDFT_FUNCTIONAL_FMT_WEIGHTED_DENSITY_H
