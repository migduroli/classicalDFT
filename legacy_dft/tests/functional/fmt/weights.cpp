...
static WeightedDensitySet make_weight_set(const std::vector<long>& shape) {
  WeightedDensitySet ws;
  ws.for_each([&](WeightedDensity& ch) { ch = WeightedDensity(shape); });
  return ws;
}

// ── Sum rules for uniform density ───────────────────────────────────────────

class WeightGeneratorUniformTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ws_ = make_weight_set(test_shape);
    WeightGenerator::generate(test_diameter, test_dx, test_shape, ws_);
    ...
    ws_.for_each([&](WeightedDensity& ch) { ch.convolve(rho_fft_.fourier()); });
  }

  WeightedDensitySet ws_;
  FourierTransform rho_fft_;
  double rho0_ = 0.8;
  double R_ = test_diameter / 2.0;
};

TEST_F(WeightGeneratorUniformTest, EtaGivesPackingFraction) {
...
TEST_F(WeightGeneratorUniformTest, ScalarGivesN2) {
...
TEST_F(WeightGeneratorUniformTest, VectorFieldsVanish) {
  for (int a = 0; a < 3; ++a) {
...
TEST_F(WeightGeneratorUniformTest, TensorDiagonalGivesN2Over3) {
...
TEST_F(WeightGeneratorUniformTest, TensorOffDiagonalVanishes) {
...
TEST_F(WeightGeneratorUniformTest, EtaMatchesMeasures) {
  auto m = FundamentalMeasures::uniform(rho0_, test_diameter);
  EXPECT_NEAR(ws_.eta.field()(0), m.eta, 1e-10);
}

TEST_F(WeightGeneratorUniformTest, ScalarMatchesMeasures) {
  auto m = FundamentalMeasures::uniform(rho0_, test_diameter);
...
TEST(WeightGenerator, DifferentDiameterGivesDifferentWeights) {
  auto ws1 = make_weight_set(test_shape);
  WeightGenerator::generate(1.0, test_dx, test_shape, ws1);

  auto ws2 = make_weight_set(test_shape);
  WeightGenerator::generate(1.5, test_dx, test_shape, ws2);

  EXPECT_NE(std::abs(ws1.eta.weight().fourier()[0]), std::abs(ws2.eta.weight().fourier()[0]));
}

TEST(WeightGenerator, ScalarDCComponent) {
  auto ws = make_weight_set(test_shape);
  WeightGenerator::generate(test_diameter, test_dx, test_shape, ws);
...
TEST(WeightGenerator, VolumeDCComponent) {
  auto ws = make_weight_set(test_shape);
  WeightGenerator::generate(test_diameter, test_dx, test_shape, ws);
...
TEST(WeightGenerator, VectorDCComponentIsZero) {
  auto ws = make_weight_set(test_shape);
  WeightGenerator::generate(test_diameter, test_dx, test_shape, ws);
...
TEST(WeightGenerator, TensorDCComponentIsotropic) {
  auto ws = make_weight_set(test_shape);
  WeightGenerator::generate(test_diameter, test_dx, test_shape, ws);
...
TEST(WeightGenerator, TensorTraceIdentity) {
  auto ws = make_weight_set(test_shape);
  WeightGenerator::generate(test_diameter, test_dx, test_shape, ws);
  ...
  ws.for_each([&](WeightedDensity& ch) { ch.convolve(rho_fft.fourier()); });
