...
TEST(FMTSpecies, ConstructionBasic) {
  FMTSpecies sp(make_density(), test_diameter);
...
TEST(FMTSpecies, ConstructionWithChemicalPotential) {
  FMTSpecies sp(make_density(), test_diameter, 2.5);
...
TEST(FMTSpecies, UniformMeasuresMatchAnalytic) {
  double rho0 = 0.5;
  FMTSpecies sp(make_density(rho0), test_diameter);

  sp.convolve_density(true);
  auto m = sp.measures_at(0);
  auto ref = FundamentalMeasures::uniform(rho0, test_diameter);
  ...
  EXPECT_NEAR(arma::norm(m.v2), 0.0, 1e-10);
