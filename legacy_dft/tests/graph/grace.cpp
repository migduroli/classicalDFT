...
TEST(grace_plot_command, set_label_xaxis_ok_test) {
  ...
  std::string actual = dft::graph::command::set_axis_label_command("test", dft::graph::Axis::X);
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_label_yaxis_ok_test) {
  std::string expected = "YAXIS LABEL \"test\"";
  std::string actual = dft::graph::command::set_axis_label_command("test", dft::graph::Axis::Y);
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_title_ok_test) {
  std::string expected = "TITLE \"test\"";
  std::string actual = dft::graph::command::set_title_command("test");
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_subtitle_ok_test) {
  std::string expected = "SUBTITLE \"subtest\"";
  std::string actual = dft::graph::command::set_subtitle_command("subtest");
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_ticks_ok_test) {
  std::string expected = "XAXIS TICK MAJOR 0.010000";
  std::string actual = dft::graph::command::set_ticks_command(0.01, dft::graph::Axis::X);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_ok_test) {
  std::string expected = "G0.S1 SYMBOL 2";
  std::string actual = dft::graph::command::set_symbol_command(dft::graph::Symbol::SQUARE, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}
...
TEST(grace_plot_command, set_symbol_color_ok_test) {
  ...
  std::string actual =
      dft::graph::command::set_symbol_color_command(dft::graph::Color::BLACK, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}
...
TEST(grace_plot_command, set_symbol_fill_color_ok_test) {
  ...
  std::string actual =
      dft::graph::command::set_symbol_color_fill_command(dft::graph::Color::BLACK, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_fill_pattern_ok_test) {
  std::string expected = "G0.S1 SYMBOL FILL PATTERN 1";
  std::string actual = dft::graph::command::set_symbol_color_fill_pattern_command(1, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}
...
TEST(grace_plot_command, set_symbol_line_type_ok_test) {
  ...
  std::string actual =
      dft::graph::command::set_line_style_command(dft::graph::LineStyle::LINE, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_size_ok_test) {
  std::string expected = "G0.S1 SYMBOL SIZE 0.500000";
  std::string actual = dft::graph::command::set_symbol_size_command(0.5, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_format_ok_test) {
  std::string expected = "HARDCOPY DEVICE \"PDF\"";
  std::string actual = dft::graph::command::set_format_command(dft::graph::ExportFormat::PDF);
  ASSERT_STREQ(expected.c_str(), actual.c_str());

  expected = "HARDCOPY DEVICE \"PNG\"";
  actual = dft::graph::command::set_format_command(dft::graph::ExportFormat::PNG);
  ASSERT_STREQ(expected.c_str(), actual.c_str());

  expected = "HARDCOPY DEVICE \"JPG\"";
  actual = dft::graph::command::set_format_command(dft::graph::ExportFormat::JPG);
  ASSERT_STREQ(expected.c_str(), actual.c_str());

  expected = "HARDCOPY DEVICE \"EPS1\"";
  actual = dft::graph::command::set_format_command(dft::graph::ExportFormat::EPS1);
  ASSERT_STREQ(expected.c_str(), actual.c_str());

  expected = "HARDCOPY DEVICE \"PS\"";
  actual = dft::graph::command::set_format_command(dft::graph::ExportFormat::PS);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, print_to_file_test_ok) {
  std::string expected = "PRINT TO \"test.t\"";
  std::string actual = dft::graph::command::print_to_file_command("test.t");
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, print_test_ok) {
  std::string expected = "PRINT";
  std::string actual = dft::graph::command::print_command();
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}
// endregion
...
TEST(grace_plot, send_command_throws_grace_not_opened_exception_test) {
  EXPECT_THROW(dft::graph::send_command("hello world"), dft::exception::GraceNotOpenedException);
}

TEST(grace_plot, error_parsing_function_prints_ok_test) {
  testing::internal::CaptureStdout();
  dft::graph::error_parsing_function("test");
...
TEST(grace_plot, start_grace_communication_negative_parameters_throws_grace_test) {
  EXPECT_THROW(dft::graph::start_grace_communication(-10, 10, 1000), dft::exception::GraceException);
...
TEST(grace_plot, get_number_of_rows_test) {
  auto actual = dft::graph::get_number_of_rows(1);
  auto expected = 1;
  ASSERT_EQ(actual, expected);

  actual = dft::graph::get_number_of_rows(2);
  expected = 1;
  ASSERT_EQ(actual, expected);

  actual = dft::graph::get_number_of_rows(10);
  expected = 2;
  ASSERT_EQ(actual, expected);
}

TEST(grace_plot, get_number_of_cols_test) {
  auto actual = dft::graph::get_number_of_columns(1, 1);
  auto expected = 1;
  ASSERT_EQ(actual, expected);

  actual = dft::graph::get_number_of_columns(2, 1);
  expected = 2;
  ASSERT_EQ(actual, expected);

  actual = dft::graph::get_number_of_columns(10, 2);
  expected = 5;
  ASSERT_EQ(actual, expected);
}

TEST(grace_plot, get_numer_of_rows_neg_or_zero_throws_except_test) {
  EXPECT_THROW(dft::graph::get_number_of_rows(-10), dft::exception::GraceException);
  EXPECT_THROW(dft::graph::get_number_of_rows(0), dft::exception::GraceException);
}

TEST(grace_plot, get_numer_of_cols_throws_except_test) {
  EXPECT_THROW(dft::graph::get_number_of_columns(-10, 2), dft::exception::GraceException);
  EXPECT_THROW(dft::graph::get_number_of_columns(0, 2), dft::exception::GraceException);
  EXPECT_THROW(dft::graph::get_number_of_columns(2, 0), dft::exception::GraceException);
  EXPECT_THROW(dft::graph::get_number_of_columns(2, -10), dft::exception::GraceException);
}
// endregion

// region Options
TEST(gace_plot_options, free_constant) {
  ASSERT_STREQ(dft::graph::option::FREE.c_str(), "-free");
}

TEST(gace_plot_options, nosafe_constant) {
  ASSERT_STREQ(dft::graph::option::NO_SAFE.c_str(), "-nosafe");
}

TEST(gace_plot_options, geometry_constant) {
  ASSERT_STREQ(dft::graph::option::GEOMETRY.c_str(), "-geometry");
}
// endregion

// region Grace class
TEST(grace_class, cttor_show_false_test) {
  auto g = dft::graph::Grace(0, 0, 0, false);
...
TEST(grace_class, getters_setter_xmin_ok_test) {
  auto g = dft::graph::Grace(10, 10, 0, false);
...
TEST(grace_class, redraw_throws_excp_graph_id_test) {
  auto g = dft::graph::Grace(10, 10, 0, true);
...
TEST(grace_class, replace_dataset_throws_excp_dataset_id_test) {
  auto g = dft::graph::Grace(10, 10, 1, true);
...
TEST(grace_class, delete_dataset_throws_excp_dataset_id) {
  auto g = dft::graph::Grace(10, 10, 1, false);
...
TEST(grace_class, set_color_throws_excp_dataset_id) {
  ...
  EXPECT_THROW(g.set_color(dft::graph::Color::BLUE, 1, 0), dft::exception::GraceException);
}
...
TEST(grace_class, set_color_throws_excp_graph_id) {
  ...
  EXPECT_THROW(g.set_color(dft::graph::Color::RED, 0, 1), dft::exception::GraceException);
}
...
TEST(grace_class, set_lable_throws_excp_graph_id) {
  ...
  EXPECT_THROW(g.set_label("test", dft::graph::Axis::X, 12), dft::exception::GraceException);

  EXPECT_THROW(g.set_label("test", dft::graph::Axis::Y, 12), dft::exception::GraceException);
}
