...
int main() {
  ...
  FMTSpecies sp(std::move(dens), 1.0);
  ...
  std::cout << "rho_min = " << dft::functional::Species::RHO_MIN << "\n";
  ...
  // ── Plot 4: f-functions ───────────────────────────────────────────────
  {
    ...
    for (int i = 0; i < N; ++i) {
      double eta = eta_vec[i];
      f1_r[i] = ros.ideal_factor(eta);
      f2_r[i] = ros.pair_factor(eta);
      f3_r[i] = ros.triplet_factor(eta);
      f3_rslt[i] = rslt.triplet_factor(eta);
      f3_wb2_v[i] = wb2.triplet_factor(eta);
    }
  ...
  {
    double c = rho_max - dft::functional::Species::RHO_MIN;
    int M = 300;
    ...
    for (int i = 0; i < M; ++i) {
      ...
      double y2 = y * y;
      y_vals[i] = y;
      rho_vals[i] = dft::functional::Species::RHO_MIN + c * y2 / (1.0 + y2);
