#ifndef DFT_GEOMETRY_BASE_ELEMENT_H
#define DFT_GEOMETRY_BASE_ELEMENT_H
...
namespace dft::geometry {
  ...
  using vertex_refwrap = std::reference_wrapper<Vertex>;
  using vertex_vec = std::vector<Vertex>;
  using vertex_map = std::unordered_map<int, vertex_refwrap>;
