#ifndef DFT_MATH_ARITHMETIC_H
#define DFT_MATH_ARITHMETIC_H
...
namespace dft::math::arithmetic {
  ...
  /**
   * @brief Standard implementation of the Kahan-Babuška-Neumaier summation algorithm
   ...
   *  recommend to read the wiki link:
   * https://en.wikipedia.org/wiki/Kahan_summation_algorithm#Further_enhancements. There you can find multiple and
   * useful references which provide details about the formal derivation, accuracy, convergence theorems etc.
   *
   * @param x_series the std::vector<double> we want to sum
   * @param sum_ini an initial value for the summation algorithm (default = 0.0)
   *  This is useful when the summation is carried out in several steps, so that we can
  ...
  /**
   * @brief Standard implementation of the Kahan-Babuška-Klein summation algorithm
  ...
  class CompensatedSum {
   private:
    ...
    /**
     * @brief The type of compensated sum to be used, from the enum class `Type`.
     *    By default set to `Type::KahanBabuskaNeumaier`
     */
    Type type_ = Type::KahanBabuskaNeumaier;
    // endregion
   ...
   public:
     ...
     */
    explicit CompensatedSum(Type type = Type::KahanBabuskaNeumaier);
    // endregion
