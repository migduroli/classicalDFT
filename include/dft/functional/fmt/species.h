#ifndef DFT_FUNCTIONAL_FMT_SPECIES_H
#define DFT_FUNCTIONAL_FMT_SPECIES_H
...
namespace dft::functional::fmt {
   ...
   *
   * Inherits `species::Species` and overrides the alias system to enforce
   * $\eta < 1$ (bounded alias). Owns a `WeightSet` whose channels compute
   ...
   */
  class Species : public species::Species {
   public:
    /**
     ...
     */
    Species(density::Density density, double diameter, double chemical_potential = 0.0);
     ...
     */
    [[nodiscard]] double compute_free_energy(const FMT& functional);
     ...
     */
    double compute_forces(const FMT& functional);
    ...
    /**
     * @brief Extract the 19 fundamental measures at lattice point @p index.
     */
    [[nodiscard]] Measures measures_at(arma::uword index) const;

    /**
     * @brief Collapse 19 derivatives back to the 11 stored derivative arrays.
     */
    void set_derivatives(const Measures& dm, arma::uword index, bool tensor);
...
#endif  // DFT_FUNCTIONAL_FMT_SPECIES_H
