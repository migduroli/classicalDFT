...
namespace dft::algorithms::dynamics {
  ...
  namespace _internal {
    ...
    inline void excess_rhs_fourier(
      ...
      arma::vec rhs(n);
      divergence_flux(rho, df_drho, rhs, nx, ny, nz, dx);
      subtract_laplacian(rho, rhs, nx, ny, nz, dx);
      rhs_ft.set_real(rhs);
      rhs_ft.forward();
    }
    ...
    inline auto apply_propagator(
        const IntegratingFactorState& st,
  ...
  [[nodiscard]] inline auto integrating_factor_step(
    ...
    _internal::excess_rhs_fourier(densities[0], forces[0], dv, nx, ny, nz, dx, rhs0);
    ...
    do {
      ...
      for (int iter = 0; iter < config.fp_max_iterations; ++iter) {
        ...
        deviation = _internal::apply_propagator(st, rho0_ft, rhs0, rhs1, d1);
  ...
  [[nodiscard]] inline auto crank_nicholson_step(
    ...
    arma::vec rhs0 = _internal::excess_rhs_real(densities[0], forces0[0], dv, nx, ny, nz, grid.dx);
