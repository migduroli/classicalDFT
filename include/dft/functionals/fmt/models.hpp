...
namespace dft::functionals::fmt {
  ...
  namespace detail {
    ...
    template <typename Model> [[nodiscard]] auto compute_phi(const Model& func, const Measures& m) -> double {
      double e = m.eta;
  ...
  template <typename Derived> struct FMTModelBase {
  ...
  struct Rosenfeld : FMTModelBase<Rosenfeld> {
    ...
    template <typename T = double> [[nodiscard]] static auto f1(T eta) -> T {
      using std::log;
      return log(T(1.0) - eta);
    }

    template <typename T = double> [[nodiscard]] static auto f2(T eta) -> T { return T(1.0) / (T(1.0) - eta); }

    template <typename T = double> [[nodiscard]] static auto f3(T eta) -> T {
      T e = T(1.0) - eta;
      return T(1.0) / (e * e);
    }
  ...
  struct WhiteBearII : FMTModelBase<WhiteBearII> {
    ...
    template <typename T = double> [[nodiscard]] static auto f2(T eta) -> T {
      using std::log;
      if (autodiff::val(eta) < 1e-6) {
  ...
  struct EsFMT : FMTModelBase<EsFMT> {
    ...
    template <typename T = double> [[nodiscard]] static auto f1(T eta) -> T { return Rosenfeld::f1(eta); }

    template <typename T = double> [[nodiscard]] static auto f2(T eta) -> T { return Rosenfeld::f2(eta); }

    template <typename T = double> [[nodiscard]] static auto f3(T eta) -> T { return Rosenfeld::f3(eta); }
