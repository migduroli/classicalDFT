...
namespace dft::physics::hard_spheres {
  ...
  template <typename T = double> [[nodiscard]] constexpr auto packing_fraction(T density) noexcept -> T {
    return (std::numbers::pi / 6.0) * density;
  }

  [[nodiscard]] constexpr auto density_from_eta(double eta) noexcept -> double {
  ...
  struct CarnahanStarling {
    ...
    template <typename T = double> [[nodiscard]] static auto excess_free_energy(T eta) -> T {
      T e = T(1.0) - eta;
