...
TEST(Measures, DefaultConstructionAllZero) {
  ...
  EXPECT_DOUBLE_EQ(arma::norm(m.v0), 0.0);
...
TEST(Measures, DefaultDerivedAllZero) {
  ...
  EXPECT_DOUBLE_EQ(m.products.dot_v0_v1, 0.0);
  EXPECT_DOUBLE_EQ(m.products.dot_v1_v1, 0.0);
  EXPECT_DOUBLE_EQ(m.products.quadratic_form, 0.0);
  EXPECT_DOUBLE_EQ(m.products.trace_T2, 0.0);
  EXPECT_DOUBLE_EQ(m.products.trace_T3, 0.0);
}
...
TEST(Measures, DotProducts) {
  ...
  EXPECT_DOUBLE_EQ(m.products.dot_v0_v1, 1.0 * 4.0 + 2.0 * 5.0 + 3.0 * 6.0);
  EXPECT_DOUBLE_EQ(m.products.dot_v1_v1, 4.0 * 4.0 + 5.0 * 5.0 + 6.0 * 6.0);
}
...
TEST(Measures, TraceT2ForDiagonalTensor) {
  ...
  EXPECT_NEAR(m.products.trace_T2, 4.0 + 9.0 + 25.0, 1e-14);
}
...
TEST(Measures, TraceT3ForDiagonalTensor) {
  ...
  EXPECT_NEAR(m.products.trace_T3, 8.0 + 27.0 + 125.0, 1e-14);
}
...
TEST(Measures, TraceT2ForFullSymmetricTensor) {
  ...
  EXPECT_NEAR(m.products.trace_T2, arma::trace(T2), 1e-12);
}
...
TEST(Measures, TraceT3ForFullSymmetricTensor) {
  ...
  EXPECT_NEAR(m.products.trace_T3, arma::trace(T3), 1e-10);
}

TEST(Measures, QuadraticFormKnownCase) {
  Measures m;
  m.v1 = {1.0, 0.0, 0.0};
  m.T = arma::eye<arma::mat>(3, 3) * 3.0;
  m.compute_inner_products();

  EXPECT_NEAR(m.products.quadratic_form, 3.0, 1e-14);
}
...
TEST(Measures, UniformDerivedConsistency) {
  ...
  EXPECT_NEAR(m.products.dot_v0_v1, 0.0, 1e-14);
  EXPECT_NEAR(m.products.dot_v1_v1, 0.0, 1e-14);
  EXPECT_NEAR(m.products.quadratic_form, 0.0, 1e-14);

  double T_diag = m.n2 / 3.0;
  EXPECT_NEAR(m.products.trace_T2, 3.0 * T_diag * T_diag, 1e-14);
  EXPECT_NEAR(m.products.trace_T3, 3.0 * T_diag * T_diag * T_diag, 1e-14);
}
