#ifdef DFT_HAS_GRACE

#include "dft/plotting/grace.h"

#include "dft/plotting/exceptions.h"

#include <gtest/gtest.h>

template <class T = double>
using Vector = std::vector<T>;

// region Default values
TEST(grace_plot, default_min_axis_value_test) {
  EXPECT_DOUBLE_EQ(dft::plotting::DEFAULT_MIN_AXIS_VALUE, 0.0);
}

TEST(grace_plot, default_max_axis_value_test) {
  EXPECT_DOUBLE_EQ(dft::plotting::DEFAULT_MAX_AXIS_VALUE, 10.0);
}

TEST(grace_plot, default_dataset_number_test) {
  EXPECT_EQ(dft::plotting::DEFAULT_DATASET_ID, 0);
}

TEST(grace_plot, default_x_y_size_test) {
  EXPECT_EQ(dft::plotting::DEFAULT_X_SIZE, 800);
  EXPECT_EQ(dft::plotting::DEFAULT_Y_SIZE, 600);
}

TEST(grace_plot, default_number_of_graphs_test) {
  EXPECT_EQ(dft::plotting::DEFAULT_NUMBER_OF_GRAPHS, 1);
}

TEST(grace_plot, default_offset_test) {
  EXPECT_FLOAT_EQ(dft::plotting::DEFAULT_OFFSET, 0.1);
}

TEST(grace_plot, default_hspace_test) {
  EXPECT_FLOAT_EQ(dft::plotting::DEFAULT_HORIZONTAL_SPACE, 0.15);
}

TEST(grace_plot, default_vspace_test) {
  EXPECT_FLOAT_EQ(dft::plotting::DEFAULT_VERTICAL_SPACE, 0.2);
}
// endregion

// region Commands
TEST(grace_plot_command, arrange_command_ok_test) {
  std::string actual = dft::plotting::command::arrange_command(1, 2, 0.3, 0.4, 0.5);
  std::string expected = "ARRANGE(1, 2, 0.300000, 0.400000, 0.500000)";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, x_min_command_ok_test) {
  std::string actual = dft::plotting::command::set_x_min_command(1.5);
  std::string expected = "WORLD XMIN 1.500000";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, x_max_command_ok_test) {
  std::string actual = dft::plotting::command::set_x_max_command(1.5);
  std::string expected = "WORLD XMAX 1.500000";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, y_min_command_ok_test) {
  std::string actual = dft::plotting::command::set_y_min_command(1.5);
  std::string expected = "WORLD YMIN 1.500000";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, y_max_command_ok_test) {
  std::string actual = dft::plotting::command::set_y_max_command(1.5);
  std::string expected = "WORLD YMAX 1.500000";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, add_point_command_ok_test) {
  std::string actual = dft::plotting::command::add_point_command(1, 2, 0, 0);
  std::string expected = "G0.S0 POINT 1.000000,2.000000";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, redraw_command_ok_test) {
  std::string actual = dft::plotting::command::redraw_command();
  std::string expected = "REDRAW";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, autoticks_command_ok_test) {
  std::string actual = dft::plotting::command::auto_ticks_command();
  std::string expected = "AUTOTICKS";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, autoscale_command_ok_test) {
  std::string actual = dft::plotting::command::auto_scale_command();
  std::string expected = "AUTOSCALE";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, focus_command_ok_test) {
  std::string actual = dft::plotting::command::focus_command(0);
  std::string expected = "FOCUS G0";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, kill_set_command_ok_test) {
  std::string actual = dft::plotting::command::kill_set_command(1, 2);
  std::string expected = "KILL G2.S1";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_legend_ok_test) {
  std::string actual = dft::plotting::command::set_legend_command("test", 1, 0);
  std::string expected = "G0.S1 LEGEND \"test\"";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_line_color_id_ok_test) {
  std::string actual = dft::plotting::command::set_line_color_command(dft::plotting::Color::RED, 1, 0);
  std::string expected = "G0.S1 LINE COLOR 2";
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_label_xaxis_ok_test) {
  std::string expected = "XAXIS LABEL \"test\"";
  std::string actual = dft::plotting::command::set_axis_label_command("test", dft::plotting::Axis::X);
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_label_yaxis_ok_test) {
  std::string expected = "YAXIS LABEL \"test\"";
  std::string actual = dft::plotting::command::set_axis_label_command("test", dft::plotting::Axis::Y);
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_title_ok_test) {
  std::string expected = "TITLE \"test\"";
  std::string actual = dft::plotting::command::set_title_command("test");
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_subtitle_ok_test) {
  std::string expected = "SUBTITLE \"subtest\"";
  std::string actual = dft::plotting::command::set_subtitle_command("subtest");
  ASSERT_STREQ(actual.c_str(), expected.c_str());
}

TEST(grace_plot_command, set_ticks_ok_test) {
  std::string expected = "XAXIS TICK MAJOR 0.010000";
  std::string actual = dft::plotting::command::set_ticks_command(0.01, dft::plotting::Axis::X);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_ok_test) {
  std::string expected = "G0.S1 SYMBOL 2";
  std::string actual = dft::plotting::command::set_symbol_command(dft::plotting::Symbol::SQUARE, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_color_ok_test) {
  std::string expected = "G0.S1 SYMBOL COLOR 1";
  std::string actual = dft::plotting::command::set_symbol_color_command(dft::plotting::Color::BLACK, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_fill_color_ok_test) {
  std::string expected = "G0.S1 SYMBOL FILL COLOR 1";
  std::string actual = dft::plotting::command::set_symbol_color_fill_command(dft::plotting::Color::BLACK, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_fill_pattern_ok_test) {
  std::string expected = "G0.S1 SYMBOL FILL PATTERN 1";
  std::string actual = dft::plotting::command::set_symbol_color_fill_pattern_command(1, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_line_type_ok_test) {
  std::string expected = "G0.S1 LINE LINESTYLE 1";
  std::string actual = dft::plotting::command::set_line_style_command(dft::plotting::LineStyle::LINE, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_symbol_size_ok_test) {
  std::string expected = "G0.S1 SYMBOL SIZE 0.500000";
  std::string actual = dft::plotting::command::set_symbol_size_command(0.5, 1, 0);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, set_format_ok_test) {
  std::string expected = "HARDCOPY DEVICE \"PDF\"";
  std::string actual = dft::plotting::command::set_format_command(dft::plotting::ExportFormat::PDF);
  ASSERT_STREQ(expected.c_str(), actual.c_str());

  expected = "HARDCOPY DEVICE \"PNG\"";
  actual = dft::plotting::command::set_format_command(dft::plotting::ExportFormat::PNG);
  ASSERT_STREQ(expected.c_str(), actual.c_str());

  expected = "HARDCOPY DEVICE \"JPG\"";
  actual = dft::plotting::command::set_format_command(dft::plotting::ExportFormat::JPG);
  ASSERT_STREQ(expected.c_str(), actual.c_str());

  expected = "HARDCOPY DEVICE \"EPS1\"";
  actual = dft::plotting::command::set_format_command(dft::plotting::ExportFormat::EPS1);
  ASSERT_STREQ(expected.c_str(), actual.c_str());

  expected = "HARDCOPY DEVICE \"PS\"";
  actual = dft::plotting::command::set_format_command(dft::plotting::ExportFormat::PS);
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, print_to_file_test_ok) {
  std::string expected = "PRINT TO \"test.t\"";
  std::string actual = dft::plotting::command::print_to_file_command("test.t");
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}

TEST(grace_plot_command, print_test_ok) {
  std::string expected = "PRINT";
  std::string actual = dft::plotting::command::print_command();
  ASSERT_STREQ(expected.c_str(), actual.c_str());
}
// endregion

// region Methods
TEST(grace_plot, send_command_throws_grace_not_opened_exception_test) {
  EXPECT_THROW(dft::plotting::send_command("hello world"), dft::exception::GraceNotOpenedException);
}

TEST(grace_plot, error_parsing_function_prints_ok_test) {
  testing::internal::CaptureStdout();
  dft::plotting::error_parsing_function("test");
  std::string output = testing::internal::GetCapturedStdout();
  auto expected_str = std::string("Grace library message: \"test\"\n");
  ASSERT_STREQ(output.c_str(), expected_str.c_str());
}

TEST(grace_plot, start_grace_communication_negative_parameters_throws_grace_test) {
  EXPECT_THROW(dft::plotting::start_grace_communication(-10, 10, 1000), dft::exception::GraceException);

  EXPECT_THROW(dft::plotting::start_grace_communication(10, -10, 1000), dft::exception::GraceException);

  EXPECT_THROW(dft::plotting::start_grace_communication(10, 10, -1000), dft::exception::GraceException);
}

TEST(grace_plot, get_number_of_rows_test) {
  auto actual = dft::plotting::get_number_of_rows(1);
  auto expected = 1;
  ASSERT_EQ(actual, expected);

  actual = dft::plotting::get_number_of_rows(2);
  expected = 1;
  ASSERT_EQ(actual, expected);

  actual = dft::plotting::get_number_of_rows(10);
  expected = 2;
  ASSERT_EQ(actual, expected);
}

TEST(grace_plot, get_number_of_cols_test) {
  auto actual = dft::plotting::get_number_of_columns(1, 1);
  auto expected = 1;
  ASSERT_EQ(actual, expected);

  actual = dft::plotting::get_number_of_columns(2, 1);
  expected = 2;
  ASSERT_EQ(actual, expected);

  actual = dft::plotting::get_number_of_columns(10, 2);
  expected = 5;
  ASSERT_EQ(actual, expected);
}

TEST(grace_plot, get_numer_of_rows_neg_or_zero_throws_except_test) {
  EXPECT_THROW(dft::plotting::get_number_of_rows(-10), dft::exception::GraceException);
  EXPECT_THROW(dft::plotting::get_number_of_rows(0), dft::exception::GraceException);
}

TEST(grace_plot, get_numer_of_cols_throws_except_test) {
  EXPECT_THROW(dft::plotting::get_number_of_columns(-10, 2), dft::exception::GraceException);
  EXPECT_THROW(dft::plotting::get_number_of_columns(0, 2), dft::exception::GraceException);
  EXPECT_THROW(dft::plotting::get_number_of_columns(2, 0), dft::exception::GraceException);
  EXPECT_THROW(dft::plotting::get_number_of_columns(2, -10), dft::exception::GraceException);
}
// endregion

// region Options
TEST(gace_plot_options, free_constant) {
  ASSERT_STREQ(dft::plotting::option::FREE.c_str(), "-free");
}

TEST(gace_plot_options, nosafe_constant) {
  ASSERT_STREQ(dft::plotting::option::NO_SAFE.c_str(), "-nosafe");
}

TEST(gace_plot_options, geometry_constant) {
  ASSERT_STREQ(dft::plotting::option::GEOMETRY.c_str(), "-geometry");
}
// endregion

// region Grace class
TEST(grace_class, cttor_show_false_test) {
  auto g = dft::plotting::Grace(0, 0, 0, false);
  ASSERT_DOUBLE_EQ(g.x_min(), 0.0);
  ASSERT_DOUBLE_EQ(g.x_max(), 10.0);
  ASSERT_DOUBLE_EQ(g.y_min(), 0.0);
  ASSERT_DOUBLE_EQ(g.y_max(), 10.0);
  ASSERT_FLOAT_EQ(g.offset(), 0.1);
  ASSERT_EQ(g.last_dataset_id(), 0);
  ASSERT_EQ(g.number_of_graphs(), 0);
  ASSERT_FLOAT_EQ(g.horizontal_space(), 0.15);
  ASSERT_FLOAT_EQ(g.vertical_space(), 0.2);
  ASSERT_FALSE(g.is_initialised());
}

TEST(grace_class, getters_setter_xmin_ok_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);
  double x_min_initial = g.x_min();

  auto expected_min = -10;
  g.set_x_min(expected_min);

  double x_min_new = g.x_min();
  ASSERT_NE(x_min_initial, x_min_new);
  ASSERT_DOUBLE_EQ(x_min_new, expected_min);
}

TEST(grace_class, getters_setter_xmax_ok_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);
  double x_max_initial = g.x_max();

  auto expected_max = 123;
  g.set_x_max(expected_max);

  double x_max_new = g.x_max();
  ASSERT_NE(x_max_initial, x_max_new);
  ASSERT_DOUBLE_EQ(x_max_new, expected_max);
}

TEST(grace_class, getters_setter_ymin_ok_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);
  double y_min_initial = g.y_min();

  auto expected_min = -10;
  g.set_y_min(expected_min);

  double y_min_new = g.y_min();
  ASSERT_NE(y_min_initial, y_min_new);
  ASSERT_DOUBLE_EQ(y_min_new, expected_min);
}

TEST(grace_class, getters_setter_ymax_ok_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);
  double y_max_initial = g.y_max();

  auto expected_max = 123;
  g.set_y_max(expected_max);

  double y_max_new = g.y_max();
  ASSERT_NE(y_max_initial, y_max_new);
  ASSERT_DOUBLE_EQ(y_max_new, expected_max);
}

TEST(grace_class, getters_setter_x_limits_ok_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);
  auto expected_min = -10;
  auto expected_max = 20;
  g.set_x_limits(expected_min, expected_max);

  double x_min = g.x_min();
  double x_max = g.x_max();
  ASSERT_DOUBLE_EQ(x_min, expected_min);
  ASSERT_DOUBLE_EQ(x_max, expected_max);
}

TEST(grace_class, getters_setter_x_limits_throws_excp_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);
  auto expected_min = 20;
  auto expected_max = 10;
  EXPECT_THROW(g.set_x_limits(expected_min, expected_max), dft::exception::GraceException);
}

TEST(grace_class, getters_setter_y_limits_ok_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);
  auto expected_min = -10;
  auto expected_max = 20;
  g.set_y_limits(expected_min, expected_max);

  double y_min = g.y_min();
  double y_max = g.y_max();
  ASSERT_DOUBLE_EQ(y_min, expected_min);
  ASSERT_DOUBLE_EQ(y_max, expected_max);
}

TEST(grace_class, getters_setter_y_limits_throws_excp_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);
  auto expected_min = 20;
  auto expected_max = 10;
  EXPECT_THROW(g.set_y_limits(expected_min, expected_max), dft::exception::GraceException);
}

TEST(grace_class, getters_setter_limits_ok_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);

  double expected_x_min = -5;
  double expected_x_max = 12;
  double expected_y_min = -10;
  double expected_y_max = 20;

  std::vector<double> x_limits{expected_x_min, expected_x_max};
  std::vector<double> y_limits{expected_y_min, expected_y_max};

  g.set_limits(x_limits, y_limits);

  auto actual_x_min = g.x_min();
  auto actual_x_max = g.x_max();
  auto actual_y_min = g.y_min();
  auto actual_y_max = g.y_max();

  ASSERT_DOUBLE_EQ(actual_x_min, expected_x_min);
  ASSERT_DOUBLE_EQ(actual_y_min, expected_y_min);
  ASSERT_DOUBLE_EQ(actual_x_max, expected_x_max);
  ASSERT_DOUBLE_EQ(actual_y_max, expected_y_max);
}

TEST(grace_class, add_point_throws_excp_test) {
  auto g = dft::plotting::Grace(10, 10, 0, false);

  EXPECT_THROW(g.add_point(1, 1, 0, 3), dft::exception::GraceException);
}

TEST(grace_class, redraw_throws_excp_graph_id_test) {
  auto g = dft::plotting::Grace(10, 10, 0, true);

  EXPECT_THROW(g.redraw(false, false, 12), dft::exception::GraceException);

  EXPECT_THROW(g.redraw(false, false, -10), dft::exception::GraceException);
}

TEST(grace_class, add_dataset_throws_excp_xy_size_test) {
  auto g = dft::plotting::Grace(10, 10, 0, true);

  auto x = Vector<>(10);
  auto y = Vector<>(12);

  EXPECT_THROW(g.add_dataset(x, y), dft::exception::GraceException);
}

TEST(grace_class, add_dataset_throws_excp_graph_id_test) {
  auto g = dft::plotting::Grace(10, 10, 0, true);

  auto x = Vector<>(10);
  auto y = Vector<>(10);

  EXPECT_THROW(g.add_dataset(x, y, 12), dft::exception::GraceException);
}

TEST(grace_class, add_dataset_works_ok) {
  auto g = dft::plotting::Grace(10, 10);

  auto x = Vector<>(10);
  auto y = Vector<>(10);

  int dataset_id = g.add_dataset(x, y);
  ASSERT_EQ(g.last_dataset_id(), dataset_id);
}

TEST(grace_class, replace_dataset_throws_excp_dataset_id_test) {
  auto g = dft::plotting::Grace(10, 10, 1, true);

  auto x = Vector<>(10);
  auto y = Vector<>(10);

  g.add_point(0, 0);
  EXPECT_THROW(g.replace_dataset(x, y, 1), dft::exception::GraceException);
}

TEST(grace_class, replace_dataset_throws_excp_graph_id_test) {
  auto g = dft::plotting::Grace(10, 10, 1, true);

  auto x = Vector<>(10);
  auto y = Vector<>(10);

  g.add_point(0, 0);
  EXPECT_THROW(g.replace_dataset(x, y, 0, 2), dft::exception::GraceException);
}

TEST(grace_class, replace_dataset_works_ok) {
  auto g = dft::plotting::Grace(10, 10, 1, true);

  auto x = Vector<>(10);
  auto y = Vector<>(10);

  g.add_point(0, 0);
  g.replace_dataset(x, y, 0, 0);

  EXPECT_EQ(0, g.last_dataset_id());
}

TEST(grace_class, delete_dataset_throws_excp_dataset_id) {
  auto g = dft::plotting::Grace(10, 10, 1, false);

  auto x = Vector<>(10);
  auto y = Vector<>(10);

  g.add_point(0, 0);
  EXPECT_THROW(g.delete_dataset(1, 0), dft::exception::GraceException);
}

TEST(grace_class, delete_dataset_throws_excp_graph_id) {
  auto g = dft::plotting::Grace(10, 10, 1, false);

  auto x = Vector<>(10);
  auto y = Vector<>(10);

  g.add_point(0, 0);
  EXPECT_THROW(g.delete_dataset(0, 1), dft::exception::GraceException);
}

TEST(grace_class, set_legend_throws_excp_dataset_id) {
  auto g = dft::plotting::Grace(10, 10, 1, false);
  g.add_point(0, 0);

  EXPECT_THROW(g.set_legend("test", 1, 0), dft::exception::GraceException);
}

TEST(grace_class, set_legend_throws_excp_graph_id) {
  auto g = dft::plotting::Grace(10, 10, 1, false);
  g.add_point(0, 0);

  EXPECT_THROW(g.set_legend("test", 0, 1), dft::exception::GraceException);
}

TEST(grace_class, set_color_throws_excp_dataset_id) {
  auto g = dft::plotting::Grace(10, 10, 1, false);
  g.add_point(0, 0);

  EXPECT_THROW(g.set_color(dft::plotting::Color::BLUE, 1, 0), dft::exception::GraceException);
}

TEST(grace_class, set_color_throws_excp_graph_id) {
  auto g = dft::plotting::Grace(10, 10, 1, false);
  g.add_point(0, 0);

  EXPECT_THROW(g.set_color(dft::plotting::Color::RED, 0, 1), dft::exception::GraceException);
}

TEST(grace_class, set_lable_throws_excp_graph_id) {
  auto g = dft::plotting::Grace(10, 10, 1, false);
  g.add_point(0, 0);

  EXPECT_THROW(g.set_label("test", dft::plotting::Axis::X, 12), dft::exception::GraceException);

  EXPECT_THROW(g.set_label("test", dft::plotting::Axis::Y, 12), dft::exception::GraceException);
}

TEST(grace_class, set_ticks_throws_excp_negative_sep) {
  auto g = dft::plotting::Grace(10, 10, 1, false);
  g.add_point(0, 0);

  EXPECT_THROW(g.set_ticks(-0.01, 0.02), dft::exception::GraceException);

  EXPECT_THROW(g.set_ticks(0.01, -0.02), dft::exception::GraceException);

  EXPECT_THROW(g.set_ticks(0.01, -0.02, 2), dft::exception::GraceException);
}
// endregion

#endif  // DFT_HAS_GRACE